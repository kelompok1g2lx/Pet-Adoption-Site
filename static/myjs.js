    ...
    $.ajax({
        ...
        success: function (response) {
            if (response["result"] === "success") {
                ...
                $("#help-id-login").text(response["msg"]);
            }
        },
    });
}
...
function sign_up() {
    ...
    let kontak = $("#input-kontak").val();
    let asal = $("#input-asal").val();
    console.log(username, password, password2, kontak, asal);
...
form_data.append("asal_give", asal);
form_data.append("kontak_give", kontak);
$.ajax({
type: "POST",
url: "/update_profile",
data: form_data,
...
function sendAdoptionRequest(id_pemilik, pet_id) {
fetch('/send_adoption_request', {
    method: 'POST',
    headers: {
...
function removeCard(petId) {
$('#pet-' + petId).remove();
}

function approve_request() {
removeCard(request.form['pet_id']);
